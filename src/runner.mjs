...
const REQUIRED_FILES = [
  ...
  'templates/auditor.md', 'templates/qc.md', 'README.md', 'package.json'
];
...
const NEXT_MAP = {
  ...
  AUDITING: { nextRole: 'qc', nextState: 'READY_FOR_QC' },
...
function cmdTransition() {
  return withLock('transition', () => {
    ...
    const state = loadState(); const prevStatus = state.status; const prevRole = state.currentRole;
...
function cmdRunPlanner(options = {}) {
  return withLock('run-planner', () => {
    ensureState(); const state = loadState();
...
function cmdRunImplementer(options = {}) {
  return withLock('run-implementer', () => {
    ...
    return log({ ok: true, status: state.status, nextRole: state.nextRole, artifact: '.agent/implementation-report.md' }, options);
  });
}
...
function cmdNewTask() {
  return withLock('new-task', () => {
    ...
    console.log(JSON.stringify({ ok: true, taskId, status: state.status, nextRole: state.nextRole, objective }, null, 2));
  });
}
...
function cmdCompleteRole() {
  return withLock('complete-role', () => {
    ...
    const mapping = transitions[role];
    if (!mapping) return fail(`Unknown role: ${role}`);
    const transition = verdict === 'pass' ? mapping.pass : mapping.fail;
    ...
    if (role === 'implementer' && state.currentIteration >= state.maxIterations && verdict !== 'pass') {
      ...
      state.lastCompletedRole = role;
      state.currentRole = verdict === 'pass' && role === 'qc' ? 'done' : 'orchestrator';
      state.status = transition.status;
      state.nextRole = transition.nextRole;
      if (role === 'implementer' && previousStatus !== 'READY_FOR_AUDIT') {
        state.currentIteration += 1;
      }
      if (verdict !== 'pass') {
        state.qualityGate.failedReasons = [`Manual ${role} completion marked as fail`];
      }
      if (state.status === 'BLOCKED') {
        applyBlock(state, `Manual ${role} completion failed`, state.qualityGate.failedReasons ?? []);
      }
    }

    saveState(state);
    appendHistory(`- ${new Date().toISOString()}: manual ${role}/${previousStatus} -> ${state.currentRole}/${state.status} (${verdict})`);
...
function cmdOpenClawAdapter() {
  ensureState(); const state = loadState();
  const adapter = { project: state.project, status: state.status, next: getNext(state), resolvedRole: resolveRoleFromState(state), roleCommandMap: { planner: 'npm run agent:run-planner', implementer: 'npm run agent:run-implementer', auditor: 'npm run agent:run-auditor', qc: 'npm run agent:run-qc' }, artifacts: REQUIRED_FILES.filter((p) => p.startsWith('.agent/')), suggestedPrompt: `Run crewctl for objective: ${state.objective}. Read .agent/workstate.json, execute the next role, update required artifacts, then report status.` };
